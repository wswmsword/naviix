  ...
  return <><AnimatePresence>
    {["l", "r"].includes(fc.resCurDir) &&
      <motion.div
        ...
        <ParallaxText baseVelocity={-13} positive={isPositive} className="fixed top-[148px] w-full py-3 -z-1 bg-[#f3c210] text-black">Naviix 由 wswmsword 强势驱动</ParallaxText></motion.div>}
  </AnimatePresence>
  <AnimatePresence>
    {["u", "d"].includes(fc.resCurDir) &&
      <motion.div
        ...
        transition={{ duration: 0 }}
        exit={{ opacity: 0, transition: { duration: 2 } }}>
        <ParallaxText baseVelocity={19} dir="col" positive={isPositive} className="fixed top-1/2 md:-translate-x-[calc(50%-325px)] rotate-90 w-full py-3 -z-1 bg-[#f3c210] text-black">Naviix ⬆️ ➡️ ⬇️ ⬅️</ParallaxText>
      </motion.div>}
  </AnimatePresence></>;
}